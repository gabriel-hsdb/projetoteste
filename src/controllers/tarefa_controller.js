...
class TarefaController {
  static async ensureAuth(req, res, next) {
    const authorization = req.headers.authorization || req.headers.authenticate;
  ...
  static async updateStatus(req, res) {
    ...
    const validProgression =
      (tarefa.status === 'CRIADO' && status === 'EM_DESENVOLVIMENTO') ||
      (tarefa.status === 'EM_DESENVOLVIMENTO' && status === 'CONCLUIDO');
